...
from NoisyUAV.modelo_v2_2_dual.model import DualStreamCVCNN
...
OUT_DIR    = r"C:\repos\DroneDetectionRF\NoisyUAV\figuras_TFM_reducidas\DualStream-DynamicSlidingWindow"
...
def calculate_phys_features(iq_tensor, adaptive_window_ms):
    """
    Extrae los 3 parámetros físicos ejecutando CFAR sobre el tensor recibido.
    Para CFAR global: pasar iq_full y adaptive_window_ms=15.0
    Para CFAR local por ventana: pasar la subventana y adaptive_window_ms=0.0
    """
    try:
        ...
        if bursts:
            ...
            # Fallback analítico: desviación máxima del ruido residual
...
def main():
    ...
    parser.add_argument('--local_cfar', action='store_true',
                        help='Usar CFAR local sobre cada ventana de 9.4 ms (Variante Dinámica)')
    ...
    mode_label = "Variante Dinámica (CFAR local)" if args.local_cfar else "DualStream-DynamicSlidingWindow (CFAR global)"
    print(f"Modo: {mode_label}")
    print(f"Cargando modelo v2.2 desde {CKPT_PATH}...")
    ...
    suffix   = "local" if args.local_cfar else "global"
    csv_path = os.path.join(out_dir, f"golden_results_dualstream_dynamic_{suffix}.csv")
    ...
    if os.path.exists(csv_path):
        ...
        for _, row in tqdm(df_golden.iterrows(), total=len(df_golden),
                           desc=f"Inferencia DualStream-Dynamic ({suffix} CFAR)"):
            ...
            try:
                ...
                if not args.local_cfar:
                    # CFAR global: se calcula una sola vez sobre los 75 ms
                ...
                for i in range(N_STEPS):
                    ...
                    if args.local_cfar:
                        # CFAR local: se recalcula sobre cada ventana de 9.4 ms
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"confusion_matrix{fig_suffix}.pdf"), **FIGSAVE_FIXED)
    plt.close(fig)
    ...
    ax.plot(recall_curve, precision_curve, linewidth=2.5, color="#4ECDC4",
            label=f"DualStream-Dynamic ({suffix}) (AP={ap:.4f} | AUC-PR={auc_pr:.4f})")
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"pr_curve{fig_suffix}.pdf"), **FIGSAVE_FIXED)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"recall_snr_lines{fig_suffix}.pdf"), **FIGSAVE_FIXED)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"accuracy_snr_bars{fig_suffix}.pdf"), **FIGSAVE_FIXED)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, f"heatmap_snr_class{fig_suffix}.pdf"), **FIGSAVE_FIXED)
    plt.close(fig)

    # Resumen en texto
    report = (
        f"=================================================\n"
        f"INFORME GOLDEN SET — DualStream-DynamicSlidingWindow ({suffix} CFAR)\n"
        f"=================================================\n"
    ...
    with open(os.path.join(out_dir, f"summary_golden_{suffix}.txt"), 'w') as fh:
        fh.write(report)
    print(report)
