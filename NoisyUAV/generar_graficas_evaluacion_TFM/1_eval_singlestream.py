...
def evaluate_test_set():
    ...
    print(f"Cargando SingleStream desde {CKPT_PATH}...")
    ...
    print(f"Test Set Original: {len(df_unique)} archivos")
    ...
    if os.path.exists(csv_path):
        ...
        results = []
        for idx, row in tqdm(df_unique.iterrows(), total=len(df_unique), desc="Evaluando Test Set"):
            ...
            global_nf      = float(np.median(nf_v)) if len(nf_v) > 0 else 0
            global_ns      = float(np.clip(ns, 0, 5))
            global_H_mean  = float(np.mean(H_smooth)) if len(H_smooth) > 0 else 0
            global_p75_act = float(np.percentile(n_active, 75)) if len(n_active) > 0 else 0
            ...
            results.append({
                'filename': row['file_path'], 'label': label_bin, 'snr': row['snr'],
                'target_multiclass': row['target_multiclass'], 'prob_max': max_prob,
    ...
    ax.set_title("Matriz de Confusión sobre el Conjunto de Test", fontweight="bold", pad=12)
