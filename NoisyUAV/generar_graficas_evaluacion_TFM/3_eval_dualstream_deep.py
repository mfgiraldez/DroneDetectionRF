...
def main():
    ...
    print(f"Evaluando DualStream-Deep (CFAR Guided) sobre GOLDEN SET ({len(df_unique)} ficheros)...")

    csv_path = os.path.join(OUT_DIR, "golden_results_dualstream_deep.csv")
    if os.path.exists(csv_path):
        print(f"Cargando resultados previos desde {csv_path}...")
        df_res = pd.read_csv(csv_path)
    else:
        results = []
        for _, row in tqdm(df_unique.iterrows(), total=len(df_unique), desc="Inferencia V4 (CFAR)"):
            ...
            try:
                ...
                prob_max = sliding_window_inference_cfar(model, iq_full, device)
    ...
    ax.plot(recall, precision, linewidth=2.5, color="#4ECDC4", 
            label=f"DualStream-Deep (AP={ap:.4f} | AUC-PR={auc_pr:.4f})")
