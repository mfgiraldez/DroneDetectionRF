...
CKPT_PATH = r"c:\repos\DroneDetectionRF\NoisyUAV\modelo_v2_1_dual_hard_test\checkpoints\best_model.pth"
OUT_DIR = r"c:\repos\DroneDetectionRF\NoisyUAV\modelo_v2_1_dual_hard_test\figuras_visualizador_ht_filtrado"
...
def visualize(target=None, snr=None, forced_filename=None, threshold=0.75, min_consecutive=2):
    ...
    print(f"Visualizando Hard Test V2.1 (Física Global): {filename}")
    ...
    titulo = (f"Hard Test V2.1 (Zero-Shot, sin T5 en train) - Evaluación \n"
              f"Archivo: {filename} \n"
    ...
    for idx, (w_start, w_end) in enumerate(window_spans):
        ...
        # Color naranja para detecciones por encima del umbral, gris para las de abajo
        w_color = '#E07C00' if prob >= threshold else '#7F8C8D'
        ...
        if prob >= threshold:
            ax2.text(times[idx], prob + 0.04, f"{prob:.2f}", color='#E07C00', fontsize=8, 
    ...
    plt.tight_layout()
    # Usar OUT_DIR definido en la configuración
    Path(OUT_DIR).mkdir(exist_ok=True, parents=True)
    out_name = os.path.join(OUT_DIR, f"diagnose_ht_filt_T{target}_SNR{snr}.png")
    plt.savefig(out_name, dpi=150)
    print(f"Diagnóstico Hard Test (Global) guardado en: {out_name}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualiza la detección del Hard Test V2.1 con lógica de post-procesado.")
