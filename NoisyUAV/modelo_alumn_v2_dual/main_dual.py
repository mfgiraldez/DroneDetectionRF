...
OUT_DIR = r"c:\repos\DroneDetectionRF\NoisyUAV\modelo_alumn_v2_dual"
CSV_PATH = os.path.join(OUT_DIR, "dataset_v5_pointers.csv")
...
def main():
    ...
    # DataLoaders estándar (sin collate personalizado)
    dl_train = DataLoader(ds_train, batch_size=32, shuffle=True, num_workers=0, pin_memory=True)
    dl_val   = DataLoader(ds_val, batch_size=32, shuffle=False, num_workers=0, pin_memory=True)
    ...
    for ep in range(start_epoch, epochs):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(OUT_DIR, "plots", "training_curves.png"), dpi=150)
        plt.close(fig)
