    ...
    with plt.rc_context(_ESTILO_PANEL):
        ...
        if ventana > 1:
            ...
            ax_env.plot(t_sub, mag_suavizada, color="#f39c12", linewidth=1.2, 
                       label=f"Envolvente suavizada (ventana={ventana})", zorder=3)

        ax_env.plot(t_sub, mag_sub, color="#2ecc71", linewidth=0.3, alpha=0.5, label="|I+jQ|")
        ...
        ax_env.set_title("2. Envolvente Temporal — Detección de Ráfagas (Bursts)", fontsize=11)
        ax_env.legend(loc="upper right", fontsize=8, framealpha=0.7)
        ax_env.grid(True)
        ...
        for i, snr_val in enumerate(snr_list):
            try:
                ...
                ax_spec.specgram(
                    ...
                    noverlap=nfft // 2, cmap="inferno", scale="dB",
                )
                ax_spec.xaxis.set_major_formatter(ticker.FuncFormatter(_fmt_ms))
                ax_spec.yaxis.set_major_formatter(ticker.FuncFormatter(_fmt_mhz))
                ...
                ax_env.plot(
                    ...
                    color="#2ecc71", linewidth=0.4, alpha=0.7,
                )
