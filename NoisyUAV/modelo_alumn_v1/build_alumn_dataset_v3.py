...
OUT_CSV       = Path(r"c:\repos\DroneDetectionRF\NoisyUAV\modelo_alumn_v1\alumn_dataset_pseudo_v3.csv")
...
# MEJORA B (v3): MAX_BINS_FRAC adaptativo por SNR
# A alto SNR (>=0) subimos el limite de bins para recuperar Targets 1, 2, 3
# cuyo ancho de banda FHSS supera el 25% a SNR muy alto.
# A bajo SNR mantenemos 0.25 para bloquear WiFi que sobrevive el umbral relajado.
def get_max_bins_frac(snr: float) -> float:
    return 0.25 if snr < 0 else 0.40

TOL_DUR = 0.20   # +-20% tolerancia de duracion FHSS (igual que v1)
...
def process_file(file_path, target_mc, snr, label, split_name,
    ...
    # MEJORA A: umbral Z_THRESH adaptativo
    z_thresh     = get_z_thresh(snr)
    # MEJORA B (v3): umbral MAX_BINS_FRAC adaptativo
    max_bins_frac = get_max_bins_frac(snr)
...
def main():
    print("=" * 65)
    print("  BUILD ALUMN DATASET V3 - Z_THRESH + MAX_BINS_FRAC adaptativos")
    print("=" * 65)
    print("  Z_THRESH  : SNR<=-10 -> 1.8 | SNR<0 -> 2.5 | SNR>=0 -> 4.0")
    print("  MAX_BINS  : SNR<0   -> 0.25 | SNR>=0 -> 0.40 (recupera T1/T2/T3)")
