"""
alumn_v3a_eval.py — Evaluacion completa del modelo Dual-Stream V3a
===================================================================
Figuras generadas (en plots_test/):
    1. heatmap_target_snr.png    -- Recall por Emisor RF x SNR
    2. heatmap_noise_snr.png     -- Especificidad por Emisor Ruido x SNR
    3. recall_snr_lines.png      -- Recall vs SNR, linea por target
    4. accuracy_per_snr.png      -- Accuracy global vs SNR
    5. pr_curve.png              -- Curva Precision-Recall
    6. confusion_matrix.png      -- Matriz de confusion
    7. accuracy_snr_bars.png     -- Barras de accuracy por SNR
    8. training_curves.png       -- Curvas de entrenamiento

Uso:
    conda activate IAIAVv3
    cd c:\\repos\\DroneDetectionRF\\NoisyUAV\\modelo_alumn_v3_dual_pro\\modelo_alumn_v3a
    python alumn_v3a_eval.py
    python alumn_v3a_eval.py --split val --out plots_val
"""
...
CSV_DEFAULT  = os.path.join(BASE_DIR, "dataset_v6_pointers.csv")
...
def load_model(ckpt_path, device):
    print(f"  Cargando: {Path(ckpt_path).name}")
    ckpt  = torch.load(ckpt_path, map_location=device, weights_only=False)
    model = DualStreamCVCNN().to(device)
    model.load_state_dict(ckpt["model_state"])
    model.eval()
    print(f"  Epoch: {ckpt.get('epoch','?')} | "
          f"Val F1: {ckpt.get('val_f1', float('nan')):.4f} | "
          f"Val Acc: {ckpt.get('val_acc', float('nan')):.4f}")
    return model
...
def run_inference(model, csv_path, data_dir, split, batch_size, device):
    ...
    for iq_b, phys_b, lbl_b in tqdm(dl, desc=f"Inferencia ({split})"):
        iq_b   = iq_b.to(device)
    ...
    result = pd.DataFrame({
        ...
        "predicted":         all_preds,
        "snr":               df_split["snr"].values[:n],
        "target_multiclass": df_split["target_multiclass"].values[:n],
    })
...
def plot_heatmap_drones(df, out_dir):
    ...
    hm = drones.pivot_table(index="Emisor RF", columns="snr",
                             values="correct", aggfunc="mean").sort_index()
    hm = hm[sorted(hm.columns)]
    fig, ax = plt.subplots(figsize=(max(14, len(hm.columns)*0.55),
                                    max(3, len(hm)*0.85)))
    ...
    ax.set_title("Dual-Stream V3a — Detección por Emisor RF y SNR (Test Set)",
                 fontsize=13, fontweight="bold", pad=12)
    ax.set_xlabel("SNR (dB)"); ax.set_ylabel("Emisor RF")
...
def plot_heatmap_noise(df, out_dir):
    ...
    hm = noise.pivot_table(index="Emisor RF", columns="snr",
                            values="correct", aggfunc="mean").sort_index()
    hm = hm[sorted(hm.columns)]
    fig, ax = plt.subplots(figsize=(max(14, len(hm.columns)*0.55),
                                    max(2, len(hm)*0.9)))
    ...
    ax.set_title("Dual-Stream V3a — Especificidad por Emisor Ruido y SNR (Test Set)",
                 fontsize=13, fontweight="bold", pad=12)
    ax.set_xlabel("SNR (dB)"); ax.set_ylabel("Emisor RF")
    ax.tick_params(axis="x", rotation=45); ax.tick_params(axis="y", rotation=0)
    fig.tight_layout()
...
def plot_recall_snr_lines(df, out_dir):
    ...
    ax.axvspan(max(-6, snr_min), min(-1, snr_max), alpha=0.07,
               color="red",    label="Grupo C")
    ax.axvspan(max(-6, snr_min), min( 9, snr_max), alpha=0.05,
               color="yellow", label="Grupo B")
    ax.axvspan(max(10, snr_min), snr_max,           alpha=0.07,
               color="green",  label="Grupo A")
    ax.set_xlabel("SNR (dB)"); ax.set_ylabel("Recall")
    ax.set_title("Dual-Stream V3a — Recall vs SNR por Emisor RF (Test Set)",
...
def plot_accuracy_per_snr(df, out_dir):
    ...
    ax.plot(snrs, acc_global, marker="D", markersize=6, linewidth=2.5,
            color="#1A1A1A", label="Accuracy Global")
    ax.plot(snrs, acc_drones, marker="o", markersize=5, linewidth=2,
            color="#0077B6", label="Recall Drones")
    ax.plot(snrs, acc_noise,  marker="s", markersize=5, linewidth=2,
            color="#D62828", linestyle="--", label="Especificidad Ruido")
    ax.axhline(0.9, color="#888", linestyle=":", linewidth=1, alpha=0.6)
    ax.axvline(0,   color="#555", linestyle="--", linewidth=1, alpha=0.5)
    ax.set_xlabel("SNR (dB)"); ax.set_ylabel("Tasa de Acierto")
    ax.set_title("Dual-Stream V3a — Accuracy por SNR (Test Set)",
                 fontsize=13, fontweight="bold")
...
def plot_pr_curve(df, out_dir):
    ...
    ax.plot(recall, precision, linewidth=2.5, color="#4ECDC4",
            label=f"Dual-Stream V3a (AP={ap:.4f} | AUC-PR={auc_pr:.4f})")
    ...
    for f1_target in [0.5, 0.6, 0.7, 0.8, 0.9]:
        ...
        v   = (p_r >= 0) & (p_r <= 1)
        ax.plot(r_r[v], p_r[v], color="#555", linewidth=0.6,
                linestyle=":", alpha=0.6)
        ax.text(r_r[v][-1] - 0.01, p_r[v][-1], f"F1={f1_target:.1f}",
                fontsize=7, color="#888", ha="right")
    ax.set_xlabel("Recall"); ax.set_ylabel("Precision")
    ax.set_title("Dual-Stream V3a — Curva Precision-Recall (Test Set)",
                 fontsize=13, fontweight="bold")
...
def plot_confusion_matrix(df, out_dir):
    cm = confusion_matrix(df["label"].values, df["predicted"].values)
    fig, ax = plt.subplots(figsize=(6, 5))
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",
                xticklabels=["Ruido (0)", "Dron (1)"],
                yticklabels=["Ruido (0)", "Dron (1)"], ax=ax)
    ax.set_title("Dual-Stream V3a — Matriz de Confusión (Test Set)",
                 fontsize=13, fontweight="bold")
...
def plot_accuracy_snr_bars(df, out_dir):
    ...
    bars = ax.bar(snrs, acc_global, edgecolor="black", alpha=0.85)
    for bar, s in zip(bars, snrs):
        bar.set_color("#E74C3C" if s < -6 else "#F1C40F" if s < 10 else "#2ECC71")
    ...
    ax.set_title("Dual-Stream V3a — Accuracy por SNR (Barras)",
                 fontsize=13, fontweight="bold")
    ax.set_ylim(0, 110); ax.set_xticks(snrs)
...
def plot_training_history(out_dir):
    ...
    ax1.set_title("Dual-Stream V3a — Curvas de Pérdida", fontweight="bold")
    ...
    ax2.set_title("Dual-Stream V3a — Evolución Métricas", fontweight="bold")
...
def print_summary_and_save(df, out_dir):
    ...
    for target in sorted(drones["target_multiclass"].unique()):
        ...
        a_acc = sub[sub["snr"] >= 10]["correct"].mean()   if len(sub[sub["snr"] >= 10])   else float("nan")
        b_acc = sub[(sub["snr"] >= -6) & (sub["snr"] < 10)]["correct"].mean() if len(sub[(sub["snr"] >= -6) & (sub["snr"] < 10)]) else float("nan")
        c_acc = sub[sub["snr"] < -6]["correct"].mean()    if len(sub[sub["snr"] < -6])    else float("nan")
    ...
    for target in sorted(noise["target_multiclass"].unique()):
        ...
        a_spec = sub[sub["snr"] >= 10]["correct"].mean() if len(sub[sub["snr"] >= 10]) else float("nan")
        c_spec = sub[sub["snr"] < -6]["correct"].mean()  if len(sub[sub["snr"] < -6])  else float("nan")
        print(f"  {name}: Spec={spec:.3f} (FP={1-spec:.3f}) | "
              f"A(>=10)={a_spec:.3f} | C(<-6)={c_spec:.3f} | n={len(sub)}")

    y_true = df["label"].values; y_pred = df["predicted"].values
    y_prob = df["prob_drone"].values
    try:
...
def parse_args():
    ...
    p.add_argument("--csv",        type=str, default=CSV_DEFAULT)
    p.add_argument("--data_dir",   type=str, default=DATA_DIR)
    p.add_argument("--out",        type=str, default=OUT_DEFAULT)
...
def main():
    ...
    print("=" * 65)
    print("  Dual-Stream V3a — Evaluación Test Set")
    ...
    df = run_inference(model, args.csv, args.data_dir,
