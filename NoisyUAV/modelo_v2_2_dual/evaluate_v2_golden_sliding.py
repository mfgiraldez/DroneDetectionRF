...
if GLOBAL_CFAR:
    OUT_DIR = r"c:\repos\DroneDetectionRF\NoisyUAV\modelo_v2_2_dual\figuras_golden_global"
else:
    OUT_DIR = r"c:\repos\DroneDetectionRF\NoisyUAV\modelo_v2_2_dual\figuras_golden_per_window"
FS = 14e6
WIN_LEN = 131072
N_STEPS = 16  # Número de ventanas deslizantes

def calculate_phys_features(iq_tensor, adaptive_window_ms):
    """ Calcula las 3 features que espera el modelo V2.2 """
    try:
...
def evaluate_golden():
    ...
    print("\n" + "="*30)
    print(f"GOLDEN SET RESULTS (V2.2)")
    ...
    report = f"""
    =================================================
    INFORME FINAL GOLDEN SET - MODELO V2.2 (SIN FILTRAR)
    =================================================
    ...
    with open(os.path.join(OUT_DIR, "summary_golden.txt"), 'w') as f:
        f.write(report)
    print(report)
    df_res.to_csv(os.path.join(OUT_DIR, "golden_results_v2_2.csv"), index=False)
