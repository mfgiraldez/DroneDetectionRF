...
1.  **Vía Temporal (Raw IQ):** Diseñada para detectar patrones de amplitud y fase a nivel de muestra. Consta de una capa de entrada de convolución 1D con un filtro gigante de integración temporal (`kernel_size=128`, `stride=4`) concebido para promediar el ruido de fondo blanco y extraer la envolvente de la señal. Le siguen capas convolucionales de `kernel_size=31` (stride 2), `kernel_size=7` y `kernel_size=3`, todas con normalización por lote y pooling, reduciendo la dimensión temporal hasta un pooling global promedio (`AdaptiveAvgPool1d(1)`) que genera un embedding temporal de 256 dimensiones. Este embedding se proyecta linealmente a 128 dimensiones (`iq_proj`).
2.  **Vía Espectral (GPU Log-PSD):** Diseñada para buscar la firma espectral de los saltos FHSS. Calcula la densidad espectral de potencia (Welch Log-PSD) en tiempo real en la GPU a partir de las muestras IQ aplicando una ventana de Hann de 131,072 muestras, ejecutando una FFT de alta resolución, aplicando un centrado espectral (`fftshift`), un agrupamiento por promedio (`avg_pool1d` con zancada y ventana de 64 muestras para reducir la dimensionalidad) y conversión a escala logarítmica ($10 \log_{10}(P + 10^{-12})$). La salida resultante ($1 \times 2048$ bins espectrales) es procesada por una CNN 1D independiente (`Conv1d(1, 32, k=15)` -> `Conv1d(32, 64, k=7)` -> `Conv1d(64, 128, k=3)`) con pooling adaptativo que produce un embedding espectral de 128 dimensiones.
...
Tras el fracaso del sobredimensionamiento analítico del modelo V4, se regresó a la arquitectura moderada y resiliente de la V2, refinando por completo el bucle de entrenamiento (AWGN) y el pipeline de inferencia temporal continua sobre el test set para fundar el modelo definitivo del proyecto.
